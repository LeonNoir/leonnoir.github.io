...
```no-line-numbers
nmap -Pn -e [имя_вашего_сетевого_интерфейса] -S 10.10.10.4 -p 5650 192.168.1.20
```
Исключение: Как nmap умеет сканировать через "чужие" IP (Idlescan)
режим Idle Scan (ключ -sI). Он позволяет узнать, открыт ли порт для 10.10.10.4, вообще не отправляя на 192.168.1.20 ни одного пакета с вашего IP.
Для этого компьютер 10.10.10.4 должен быть в сети (хотя бы его роутер) и не генерировать другого трафика. nmap отправляет пакеты к 10.10.10.4, следит за изменением счетчиков ID пакетов (IPID) в его операционной системе, затем шлет спуф-пакет на 192.168.1.20 от его имени, и по изменению счетчиков у 10.10.10.4 понимает, принял 192.168.1.20 пакет или дропнул.
